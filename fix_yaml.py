...
if __name__ == '__main__': 
    ...
    if os.path.isfile(args.input): 
        ...
        assert args.input.endswith('.nrrd') or args.input.endswith('.yaml'), "Please specify path to .nrrd or .yaml file, or to a directory"
        
        # must specify input as .yaml file
        if args.input.endswith('.nrrd'):             
...
# os.path.isdir(args.input):
        ...
        files = glob.glob(args.input + "*.yaml")
    ...
    for file in files: 
        ...
        if proceed: 
            ...
            if args.outdir is not None:
                outdir = args.outdir + "/" if not args.outdir.endswith("/") else args.outdir
            ...
            if multi_echo_file and args.outdir is not None: 
                # copy nrrd file 
                shutil.copyfile(file, args.outdir+"/" + os.path.basename(file))
                # copy yaml file 
